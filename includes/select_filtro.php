<?php
function select_filtro($tabla,$orden,$nombre_select,$actual,$predeterminado,$campo_id,$matriz_campos,$filtro,$habilitado=false)
{
	if($habilitado){
		$disabled="disabled";
	}else{
	...
	if ($filtro)
	{
		$filtro_sql=$filtro. ' AND ';
	}
	else
	{
		$filtro_sql='';
	}
	$sql_select="SELECT * FROM $tabla WHERE $filtro_sql eliminado=0 $orden_sql;";
